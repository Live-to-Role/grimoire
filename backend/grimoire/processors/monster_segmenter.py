...
def segment_pages(pages: list[dict], profile: SystemProfile) -> list[Candidate]:
    ...
    for page_dict in pages:
        ...
        for pos, (start, anchor_idx) in enumerate(zip(starts, anchor_lines)):
            ...
            # and never let it clip away the anchor's own statline either:
            ...
            # and a rescued candidate must always include at least the line
            # that anchored it -- a name-only block (e.g. just a header) is
            # invisible to the downstream LLM normalizer and the monster is
            # still effectively lost even though the slice is non-empty.
            end = max(anchor_idx + 1, start + 1, end)
